...
void Game::Init()
{
	...
	// wczytanie tekstury
	tex = Content::LoadTexture("../resources/czajnik.png");
	//tex = Content::LoadTexture("../resources/tekstury.png");

	// wczytanie modelu
	testModel = Content::LoadModel( "../resources/czajnik.ASE" );
	//testModel = Content::LoadModel( "../resources/auto.ASE" );
	...
	//model 
	autoModel = Content::LoadModel("../resources/cow.obj");
	autoTekstura = Content::LoadTexture("../resource/cow.jpg");
	glm::mat4x4 nn;
	for (int i = 0; i < autoModel->Elements(); i++)
	{
		//ustawienie koloru materia³u dla pobranego modelu
		autoModel->GetMesh(i)->GetMaterial().Color = glm::vec3(1, 1, 0);
		//podmiana tekstury w modelu
		autoModel->GetMesh(i)->GetMaterial().texture = autoTekstura;

		nn = autoModel->GetMesh(i)->getLocalWorld();
	}
...
void Game::Update()
{
	...
	if (Keyboard::isPressed(GLFW_MOUSE_BUTTON_RIGHT)) rotate -= 0.8f;
	if (Keyboard::isPressed(GLFW_MOUSE_BUTTON_LEFT)) rotate += 0.8f;


	//P-pocisk
	if (Keyboard::isPressed('Q')) {
		pociskrotate = rotate;
		pociskX = posX + speed * (float)cos(pociskrotate * 3.14 / 180.0f);
		pociskY = posY - speed * (float)cos(pociskrotate * 3.14 / 180.0f);
		pocisk = true;
	}
...
void Game::drawScene( void ) {
	...
	this->shaderColor->GetParameter("World")->SetValue(
		...
		glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 15.0f, 0.0f)) *
		glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		glm::scale(glm::mat4(1.0f), glm::vec3(0.5f, 0.005f, 0.2f))
	);
	// rysowanie modelu
	...
	this->shaderColor->GetParameter("World")->SetValue(
		...
		glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		glm::scale(glm::mat4(1.0f), glm::vec3(0.01f, 0.5f, 0.2f))
	);
	// rysowanie modelu
	sceneModelBox->GetMesh(i)->Draw();
	...
	this->shaderColor->GetParameter("World")->SetValue(
		sceneModelBox->GetMesh(i)->getLocalWorld() *
		glm::translate(glm::mat4(1.0f), glm::vec3(-6.5f, 0.0f, 0.0f)) *
		glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		glm::scale(glm::mat4(1.0f), glm::vec3(0.01f, 0.5f, 0.2f))
	);
	// rysowanie modelu
	...
	this->shaderColor->GetParameter("World")->SetValue(
		sceneModelBox->GetMesh(i)->getLocalWorld() *
		glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.5f, -3.0f)) *
		glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		glm::scale(glm::mat4(1.0f), glm::vec3(0.5f, 0.5f, 0.01f))
	);
	// rysowanie modelu
	sceneModelBox->GetMesh(i)->Draw();
	...
	if (kolizja) {
		this->shaderColor->GetParameter("World")->SetValue(
			sceneModelBox->GetMesh(i)->getLocalWorld() *
			glm::translate(glm::mat4(1.0f), glm::vec3(-17.0f, -12.0f, 0.3f)) *
			glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(0.5f, 0.0f, 0.0f)) *
			glm::scale(glm::mat4(1.0f), glm::vec3(0.1f, 0.7f, 0.2f))
		);
		sceneModelBox->GetMesh(i)->Draw();
	}
	...
	if (pocisk) {
		this->shaderColor->GetParameter("World")->SetValue(
			sceneModelBox->GetMesh(i)->getLocalWorld() *
			glm::translate(glm::mat4(1.0f), glm::vec3(pociskX, pociskY, 0.0f)) *
			glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
			glm::scale(glm::mat4(1.0f), glm::vec3(0.01f, 0.01f, 0.01f))
		);
		sceneModelBox->GetMesh(i)->Draw();
		pociskX += speed * (float)cos(pociskrotate * 3.14 / 180.0f);
		pociskY -= speed * (float)sin(pociskrotate * 3.14 / 180.0f);
	}

	
	//model
	shaderColor->GetParameter("matColor")->SetValue(autoModel->GetMesh(i)->GetMaterial().Color);
	this->shaderColor->GetParameter("World")->SetValue(
		autoModel->GetMesh(i)->getLocalWorld() *
		glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 4.0f, -2.0f)) *
		glm::rotate(glm::mat4(1.0f), 90.0f, glm::vec3(1.0f, 0.0f, 0.0f)) *
		glm::scale(glm::mat4(1.0f), glm::vec3(0.05f, 0.05f, 0.05f))
	);
	autoModel->GetMesh(i)->Draw();
	
	if (pociskX > -17.0f && pociskX < -16.0f){
		kolizja = false;
		pocisk = false;
	}
}
...
void Game::Redraw()
{
	...
	//--------------------auto----------------
	shaderColor->GetParameter("matColor")->SetValue(autoModel->GetMesh(i)->GetMaterial().Color);
	if (autoModel->GetMesh(i)->GetMaterial().texture != NULL)
	{
		//jeœli mamy teksturê to j¹ bindujemy i ustawiamy tryb teskturowania w shaderze
		shaderColor->GetParameter("mode")->SetValue(1.0f);
		shaderColor->GetParameter("tex")->SetValue(*autoModel->GetMesh(i)->GetMaterial().texture);
	}
	else
	{
		//w³¹czenie trybu bez tekstury
